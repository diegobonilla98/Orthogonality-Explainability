...
def setup_distributed(cfg: TrainConfig) -> TrainConfig:
    ...
    if torch.cuda.is_available():
        ...
        if num_gpus > 1:
            ...
            # Ensure rendezvous env vars for env:// are set (single-node default)
...
# ------------------------------
# Model helpers (activation capture)
# ------------------------------
def _register_layer4_hook(model: nn.Module):
    """Register forward hook on layer4, supporting DDP-wrapped models.

    If the model is wrapped in DistributedDataParallel, access the
    underlying module via `.module` before registering the hook.
    """
    cache = {'A': None}
    def hook(module, input, output):
        cache['A'] = output.detach()
    target_model = model.module if isinstance(model, DDP) else model
    handle = target_model.layer4.register_forward_hook(hook)
    return cache, handle
...
def train_worker(rank: int, cfg: TrainConfig):
    ...
    # Only rank 0 handles dataset download
    if rank == 0:
    ...
    # Standard torchvision resnet18 (no layer modifications)
    model = resnet18(weights=ResNet18_Weights.IMAGENET1K_V1 if cfg.pretrained else None)
